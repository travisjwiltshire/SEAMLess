...
## The rest of this chunk does not need to be changed
...
## This determines which user data is collected  
...
An example of a basic question and plot.
...
## You can add or remove answer options by copypasting another line of answer("answer option"), or deleting one of the lines below
  ...
  question("Which of the following models shows orthogonal polynomial regression with a quadratic term?", ## Your question here
    answer("`lm(affect ~ stress + I(stress^2))`"),
    answer("`lm(affect ~ stress * performance)`"),
    answer("`lm(affect ~ poly(stress, 2) + performance)`", correct = TRUE), ## Move ", correct = TRUE" after the correct answer
    answer("`lm(affect ~ poly(stress, 3) + performance)`"),
    allow_retry = FALSE ## Should the students be able to try again? FALSE = only one attempt, TRUE = unlimited attempts
    )
```
