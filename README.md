...
To run a template intended for local testing [Quizzes without plots](../SEAMLess/Templates/Quizzes without plots), download the folder and open the .Rmd file in RStudio. Ensure you have installed the required packages (rmarkdown, shiny, learnr), you can do this with `install.packages("package_name")` if needed.
